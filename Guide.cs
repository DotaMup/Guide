...
namespace Guide
{
    class Guide
    {
        const string Staff = "Notebook.txt";
        ...
        static void Input()
        {
            ...
            int id = 1;
            if (!File.Exists(Staff))
            {
                File.Create(Staff).Close();
                Console.WriteLine("Файл создан");
            }
            else
            {
                id = File.ReadAllLines(Staff).Length + 1;
            }
        ...
        static void Print()
        {
            ...
            using (StreamReader list2 = new StreamReader(Staff, Encoding.Unicode))
            {
                string line;
                Console.WriteLine($"{"Id",5}{"Дата и время",20}{"Ф.И.О",30} {"Возраст",15} {"Рост",15} {"Дата Рождения",15} {"Место",20}");
                while ((line = list2.ReadLine()) != null)
                {
                    string[] daty = line.Split('#');
                    Console.WriteLine($"{daty[0],5}{daty[1],20} {daty[2],30} {daty[3],15} {daty[4],15} {daty[5],15} {daty[6],20}");
                }
            }
        }
    }
